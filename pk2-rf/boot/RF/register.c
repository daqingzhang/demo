...
void response_bb(const int index,const int code,const int location){
    ...
    if(0xa0==location){
        ...
        LogRecord.IntLogBuff[LogRecord.BIndex]=((TimeTrace<<8)|((location|(index&0x1f))&0xff))&0xffffffff;
        LogRecord.IntEveBuff[index]=((TimeTrace<<8)|((location|(index&0x1f))&0xff))&0xffffffff;
